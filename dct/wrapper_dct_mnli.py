...
class TransformerModelWrapper:
    ...
    def train(self,
        ...
        for _ in train_iterator:
            ...
            self.eval_han(eval_data, eval_config, n_gpu)
    ...
    def han_test(self, predictions):
        guess_dict = {}
        for i in range(len(predictions)):
            if predictions[i] == 0:
                guess_dict['ex' + str(i)] = 'entailment'
            else:
                guess_dict['ex' + str(i)] = "non-entailment"
        fi = open("./PATH_TO_DATA_DIR/MNLI/heuristics_evaluation_set.txt", "r")
        ...
        logger.info("")
        logger.info("Heuristic entailed results:")
        results_e = 0
        for heuristic in heuristic_list:
            ...
            results_e += percent
            logger.info(heuristic + ": " + str(percent))
        logger.info('HAN_E: {:.2f}% '.format(results_e * 100 / 3))
        logger.info("")
        logger.info("Heuristic non-entailed results:")
        results_ne = 0
        for heuristic in heuristic_list:
            ...
            results_ne += percent
            logger.info(heuristic + ": " + str(percent))
        logger.info('HAN_NE: {:.2f}% '.format(results_ne * 100 / 3))
        logger.info("ALL results:")
        logger.info('HAN: {:.2f}% '.format((results_e + results_ne) * 100 / 6))
        logger.info("")

    def eval_han(self, dev_data, eval_config, n_gpu):
    ...
    def _generate_dataset_train_begin(self, data: List[InputExample], labelled: bool = True):
        ...
        #add hard neg
        hard_index = np.load('./pos_neg_index/MNLI/hard_index.npy')
        hard_features = []
    ...
    def _generate_dataset_train_epoch(self, epoch):
        ...
        mnli_hard_neg_index=np.load('./pos_neg_index/MNLI/mnli_hard_tbert_negindex_epoch'+str(epoch+1)+'.npy')
        feature_dict = {
