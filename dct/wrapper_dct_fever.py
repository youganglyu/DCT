...
class TransformerModelWrapper:
    ...
    def only_eval(self,
        ...
        dev_scores = self.eval_dev(dev_data, eval_config, n_gpu)
        eval1_scores = self.eval_val1(eval_data, eval_config, n_gpu)
        eval2_scores = self.eval_val2(eval_data2, eval_config, n_gpu)
    ...
    def eval_dev(self, dev_data, eval_config, n_gpu):
        ...
        return scores
    def eval_val1(self, dev_data, eval_config, n_gpu):
        self.model.eval()
        results = self.eval(dev_data,
                            per_gpu_eval_batch_size=eval_config.per_gpu_eval_batch_size,
                            n_gpu=n_gpu)
        ...
        for metric in metrics:
            if metric == 'acc':
                ...
                raise ValueError(f"Metric '{metric}' not implemented")
        logger.info("FEVER val1: {:.2f}%".format(scores['acc'] * 100))
        logger.info('---')
        # logger.info("MNLI ACCU {:.2f}%".format(scores['acc']*100))
        return scores

    def eval_val2(self, dev_data, eval_config, n_gpu):
        self.model.eval()
        results = self.eval(dev_data,
                            per_gpu_eval_batch_size=eval_config.per_gpu_eval_batch_size,
                            n_gpu=n_gpu)
        ...
        for metric in metrics:
            if metric == 'acc':
                ...
                raise ValueError(f"Metric '{metric}' not implemented")
        logger.info("FEVER val2: {:.2f}%".format(scores['acc'] * 100))
        logger.info('---')
        # logger.info("MNLI ACCU {:.2f}%".format(scores['acc']*100))
        return scores
    ...
    def _generate_dataset_train_begin(self, data: List[InputExample], labelled: bool = True):
        ...
        #add hard neg
        hard_index = np.load('./pos_neg_index/FEVER/hard_index_fever.npy')
        hard_features = []
    ...
    def _generate_dataset_train_epoch(self, epoch):
        ...
        mnli_hard_neg_index=np.load('./pos_neg_index/FEVER/fever_hard_tbert_negindex_epoch'+str(epoch+1)+'.npy')
        feature_dict = {
